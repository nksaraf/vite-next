...
export async function ssrBuild(
  ...
  const ssrOutput = await viteBuild({
    ...
    build: {
      ...
      cssCodeSplit: false,
      rollupOptions: {
        input: path.join(ROOT_PATH, 'render', 'server.tsx'),
  ...
  const _clientResult = await viteBuild({
    ...
    build: {
      cssCodeSplit: false,
      rollupOptions: {
        input: path.join(ROOT_PATH, 'render', 'client.tsx'),
        preserveEntrySignatures: 'allow-extension',
      },
      assetsDir: '_assets',
      outDir: clientOutDir,
  ...
  const EntryModuleInjectPoint =
    '<script type="module" src="/@pages-infra/main.tsx"></script>'
  ...
  await Promise.all(
    pagePaths.map(async (pagePath) => {
      ...
      let html = htmlCode.replace(
        ...
        `<script>window.__NEXT_DATA__=${JSON.stringify(ssrInfo)};</script>
